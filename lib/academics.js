...
const LIST_PUBLIC_ID = "site-data/academics";
const IMAGE_FOLDER = "academics";
...
export async function getAcademicsPhotos() {
  const list = await readList();
  return [...list].sort((a, b) => (b.uploadedAt || 0) - (a.uploadedAt || 0));
}

export async function addAcademicsPhoto(file, caption = "") {
...
export async function removeAcademicsPhoto(id) {
  const list = await readList();
